...
// ---------------------------------------------------------------------------
// Ticket 33 -- the moving-entry movement capture.
//
// This is NOT the ticket-19 capture c73b4f1 retired. That one keyed on SH2's own
// cast-type `blocks_movement` chain, i.e. on what the DLL believed it had started;
// ADR-0015 retired it because the shtb states are FULL-BODY -- entering one exits
// locomotion, so the state's own bAnimationDriven plant is the whole root and a
// second DLL-side root was duplication that could only fight it.
//
// The vanilla concentration states ticket 33's `shcc` patch roots are LAYERED over
// live locomotion instead (that is why vanilla walks while channeling). There the
// plant roots the controller while held movement keys keep feeding the still-running
// locomotion layer, and the two fight: the owner's moving entry stutters, slides on
// entry momentum, and stops late. No behavior-graph primitive gates the player
// controller's input while the locomotion layer runs, so the block is DLL-owned --
// ADR-0015's 2026-08-29 amendment. The NPC side stays behavior-only (proven green):
// NPC AI stops issuing movement intent when it casts, so there is nothing to swallow.
//
// The gate below reads the LIVE ENGINE STATE, not SH2's cast bookkeeping, which is
// why it also covers vanilla equipped-hand casts SH2 never drives.

// Caster states this treats as an active cast, as raw values of RE::MagicCaster::State.
// A concentration channel reports kCasting for its whole length and kCharging on the
// way in; the two together are the span the root covers. Mirrored as plain ints so the
// predicate stays testable without CommonLibSSE; input.cpp static_asserts them against
// the real enum, so an upstream renumbering breaks the build rather than the gate.
inline constexpr int k_magic_caster_state_charging = 5;  // RE::MagicCaster::State::kCharging
inline constexpr int k_magic_caster_state_casting = 6;   // RE::MagicCaster::State::kCasting

[[nodiscard]] constexpr bool magic_caster_state_is_actively_casting(int state) noexcept
{
	return state == k_magic_caster_state_charging || state == k_magic_caster_state_casting;
}

// One equipped hand contributes to the gate only when its caster is mid-cast AND the
// spell in it is a concentration spell. Either half alone is the wrong actor: a
// fire-and-forget charge is not a channel, and a concentration spell merely equipped
// is not being cast.
[[nodiscard]] constexpr bool hand_holds_active_concentration_cast(
	bool caster_state_active, bool spell_is_concentration) noexcept
{
	return caster_state_active && spell_is_concentration;
}

// THE GATE. Both halves, and each one is load-bearing:
//
//  * the concentration cast, because bAnimationDriven alone is not ours -- an MCO
//    attack sets it too, and swallowing movement there would change attack feel;
//  * bAnimationDriven, because a concentration cast alone is not rooted -- a user
//    without the optional `shcc` patch channels with the flag false, and walk-casting
//    must stay vanilla for them.
//
// Hotbar-driven concentration channels satisfy both and are swallowed too. That is
// harmless rather than intended: SH2_Channel_State is full-body, so locomotion is
// already gone and the movement input it swallows was inert.
//
// Fails OPEN by construction: every caller that cannot read a caster, a spell, or the
// graph passes false and movement travels untouched.
[[nodiscard]] constexpr bool concentration_cast_swallows_movement(
	bool equipped_hand_concentration_cast_active, bool animation_driven) noexcept
{
	return equipped_hand_concentration_cast_active && animation_driven;
}
