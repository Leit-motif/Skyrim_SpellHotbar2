...
namespace SpellHotbar::Lifecycle {
    namespace {
        // Vanilla SOUN 000362B6 MAGCloakIn and 0003966B MAGCloakOut, the descriptors the
        // retired SpellHotbarToggleDualCastingEffect's SoundToggleOn/SoundToggleOff properties
        // pointed at.
        ...
        // togglePlayerKnowsPower use: 0 the unbind-slot power, 1 the dual-cast toggle.
        ...
        // SpellHotbarBattleMageInitQuestScript granted a power whose effect script opened the
        // tree. The tree opens from the config tool now, so a save that still carries the
        // power loses it.
        void remove_legacy_battlemage_power()
        {
            ...
            if (player != nullptr && legacy_power != nullptr && player->HasSpell(legacy_power)) {
                ...
                logger::info("Removed legacy BattleMage opener power; the tree opens from the config tool");
            }
        }
    ...
    void on_player_in_world()
    {
        if (pending_first_init) {
            try_pending_first_initialization();
            return;
        }
    ...
    // What SpellHotbarOpenBattleMagePerkTree did from Papyrus: CustomSkills.OpenCustomSkillMenu
