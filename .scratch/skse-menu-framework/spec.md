...
27. As a player, I want Driver Cast, Ability slots, and existing co-save format 6 to load unchanged, and new saves to write format 7 (`spell_gcd`), so that this is not a save break.
...
- **Domain stays SH2:** settings objects, keybinds, bars, GameData, Storage co-save format 7 (format 6 still loads Ability slots; format 7 adds `GameData::spell_gcd`), JSON preset format 2, Direct Cast / input modes, Magic Menu / favorites / inventory bind gate, Ability slots, Weapon Arts, Driver Cast / Cast Channel, ShoutMCO.
...
- **Existing (keep):** SKSE co-save format 7 and JSON preset format 2 round-trip. Format 5 and 6 saves must still load. MCP writes must be visible after save/load the same way MCM writes were. Ability binds, Weapon Arts, cooldown/GCD data, and `auto_profile.json` are part of that persistence, not extras.
