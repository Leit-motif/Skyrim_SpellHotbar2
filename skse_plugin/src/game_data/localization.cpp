...
namespace SpellHotbar {
	...
	std::unordered_map<std::string, std::string> _get_default_translations()
	{
		std::unordered_map<std::string, std::string> t = {
			...
			{"$TAB_TEXT_FOOD",  "Food"},
			{"$TAB_TEXT_ARTS",  "Abilities"},
			{"$TAB_TEXT_ACTIONS", "Actions"},
			...
			{ "$EDIT_ICON", "Edit Icon" },
			{ "$ABILITY_EDITOR", "Ability Editor" },
			{ "$ACTION_EDITOR", "Action Editor" },
			{ "$ART_CLASS", "Ability Class" },
			{ "$ACTION_KIND", "Kind" },
			{ "$ACTION_KIND_PHYSICAL", "Physical scancode" },
			{ "$ACTION_TARGET", "Target" },
			{ "$ACTION_TARGET_OCPA", "OCPA power attack" },
			{ "$ACTION_TARGET_DODGE", "Dodge hotkey" },
			{ "$ACTION_TARGET_CAPTURED", "Captured scancode" },
			{ "$ACTION_SCANCODE", "Scancode" },
			{ "$ACTION_DEVICE_KEYBOARD", "Keyboard" },
			{ "$ACTION_DEVICE_MOUSE", "Mouse" },
			{ "$ACTION_DEVICE_GAMEPAD", "Gamepad" },
