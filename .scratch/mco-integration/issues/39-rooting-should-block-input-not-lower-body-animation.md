...
### Second addendum: the rotation itself was wrong, and it lived in the pack files

Owner correction after the traverse fix: SH2 stepped on casts 1, 2, 3 while a natty MSCO cast
steps only on 1 and 3. Root cause: MSCO's rotation is authored as `@SGVI` successor payloads in
the clips (aimed left AND right both walk 1 -> 5 -> 3 -> 4, where clip 5 is clip 2's
motion-stripped twin — same duration, same SpellFire at 0.300; the self sets alternate
1,2,1,2; staff and dual walk 1,2,3,4), while SH2's `CastComboIndex` is a plain 1-2-3-4 wrap
over pack file positions. Ticket 46/60's pack copied source clips 1..4 by NAME, so position 2
carried moving art the native rotation never plays. Fix: pack positions now carry the
rotation — aimed position 2 is the family's clip 5, self position 4 is clip B — generated by
`python_scripts/fix_cast_rotation_files.py` (stamps per ticket 60's conventions,
hkxc-verified), commit `d5855ab`. Measured live after redeploy: 45.0 / 0.0 / 60.9 / 0.0 units
across the four-cast rotation — natty parity, casts 1 and 3 step.
